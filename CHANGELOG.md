...
- Elements not in Jaspr's DOM helpers are implemented via `Component.element()` directly
- **Document elements:** `Html`, `Head`, `Body`, `Title`, `StyleElement`, `Base`, `Meta`, `HtmlLink`, `Noscript`
- **Text elements:** `Abbr`, `Bdi`, `Bdo`, `Cite`, `HtmlData`, `Dfn`, `Kbd`, `Mark`, `Q`, `Rp`, `Rt`, `Ruby`, `Samp`, `Sub`, `Sup`, `Time`, `HtmlVar`, `Del`, `Ins`
- **Media elements:** `Picture`, `Canvas`, `Track`, `ImageMap`, `Area`, `ObjectEmbed`
- **Semantic elements:** `Address`, `Hgroup`, `Search`
- **Content elements:** `Menu`
- **Form elements:** `Output`
- **Web Component elements:** `HtmlTemplate`, `Slot`
